...
export default function Dashboard() {
  ...
  const loadServiceProviderData = async (userId) => {
    ...
    let myServices = allServices.filter(
      (s) => s.service_provider_id === userId
    );
    ...
    setStats({
      ...
      activeServices: myServices.filter((s) => s.status === "in_progress")
        .length,
      completedServices: myServices.filter((s) => s.status === "completed")
        .length,
      totalEarnings: myPayments.reduce(
        (sum, p) => sum + (p.net_amount || p.amount || 0),
        0
      ),
  ...
  const loadVehicleOwnerData = async (userId) => {
    ...
    setStats({
      totalServices: myServices.length,
      pendingServices: myServices.filter((s) => s.status !== "completed")
        .length,
      completedServices: myServices.filter((s) => s.status === "completed")
        .length,
      totalSpent: myServices.reduce(
    ...
    setRecentActivity(
      myServices.slice(0, 6).map((s) => ({
        type: "service",
        title: `${s.service_type?.replace(/_/g, " ") || "Service"} - ${
          s.status
        }`,
        time: s.created_date,
        status: s.status,
  ...
  const loadPaymentCollectorData = async (userId) => {
    ...
    setStats({
      ...
      amountCollected: payments
        .filter((p) => p.payment_status === "collected")
        .reduce((sum, p) => sum + (p.amount || 0), 0),
      pendingCollections: payments.filter((p) => p.payment_status === "pending")
        .length,
      failedCollections: payments.filter((p) => p.payment_status === "failed")
        .length,
    });
  ...
  const loadWarehouseData = async (userId) => {
    ...
    setStats({
      ...
      packedToday: orders.filter(
        (o) =>
          o.packed_by === userId &&
          new Date(o.packed_date).toDateString() === new Date().toDateString()
      ).length,
  ...
  const loadDispatcherData = async (userId) => {
    ...
    setStats({
      ...
      dispatchedToday: orders.filter(
        (o) =>
          o.dispatched_by === userId &&
          new Date(o.dispatched_date).toDateString() ===
  ...
  const loadInsuranceAgentData = async (userId) => {
    ...
    setStats({
      ...
      totalCommission: leads
        .filter((l) => l.converted_policy)
        .reduce(
          (sum, l) => sum + (l.converted_policy.commission_earned || 0),
          0
        ),
    });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600 mt-1">
            ...
            {user.user_type?.replace("_", " ") || "account"} dashboard
          </p>
        </div>
